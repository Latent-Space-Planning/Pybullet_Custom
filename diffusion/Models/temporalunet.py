...
class TemporalUNet(nn.Module):
    ...
    def __init__(self, model_name, input_dim, time_dim, dims = (32, 64, 128, 256)):
        ...
        for i in range(len(dims) - 2):      # Loops 0, 1, 2
            # print(f"Downsampling layer {i}\t Dims: {dims[i]}->{dims[i+1]}")
            self.down_samplers.append(DownSampler(dims[i], dims[i+1], time_dim))
        # print(f"Downsampling layer {len(dims) - 2}\t Dims: {dims[-2]}->{dims[-1]}")
        ...
        # self.up_samplers.append(UpSampler(dims[-2], dims[-1], time_dim, is_last = True))  # 4 -> 3
        for i in range(len(dims) - 1, 1, -1):  # Loops 4, 3, 2  since the last one is a seperate convolution
            # print(f"Upsampling layer {i}\t Dims: {dims[i-1]}->{dims[i]}")
    ...
    def forward(self, x, t):
        ...
        for i in range(len(self.down_samplers)):
            ...
            # print(f"Shape of x: {x.shape}\t Shape of h: {h.shape}")
            h_list.append(h)
