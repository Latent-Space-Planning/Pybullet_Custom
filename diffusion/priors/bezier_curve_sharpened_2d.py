...
def generate_bezier_curves(num_samples, traj_len):
    ...
    P3 = np.random.uniform(low = 0.001, high = 1, size = (num_samples, 1))
    P3 = np.concatenate([P3, np.zeros((num_samples, 1))], axis = 1)

    P1_low = np.repeat([[0, -1]], num_samples, axis = 0)
    P1_high = np.ones((num_samples, 2))
    P1_high[:, 0] = P3[:, 0]
    P1 = np.random.uniform(low = P1_low, high = P1_high, size = (num_samples, 2))

    P2_low = np.concatenate([[P1[:, 0]], np.zeros((1, num_samples))]).T
    P2_high = np.concatenate([[P3[:, 0]], np.sign([P1[:, 1]])]).T
