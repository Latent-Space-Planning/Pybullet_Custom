...
dists = np.zeros((trajectory.shape[0], trajectory.shape[1]), dtype=float)  #2D matrix , verify in pybullet
for conf_no in range(len(trajectory)):  #50 for mpinets
    ...
    for joint_no in range(7):# numJoints):
        ...
        for obstacle_no in range(len(obstacle_centers)):
            ...
            # print(value)
            dists_per_joint.append(value)
        
        dists[conf_no, joint_no] = min(dists_per_joint)
    
    print('**************')

print('dits shape: {}'.format(dists.shape))
file_name = "sdf_dists_three_spheres_min_distance_per_joint.npy"
