...
for i in range(len(obstacle_centers)):
    (obstacles_bbox_vertices[i], obstacles_bbox_centers[i], obstacles_bbox_dims[i]) = find_cuboid_bbox_parameters(obstacle_centers[i], 0.08)

print('obstacles bbox vertices shape: {}'.format(obstacles_bbox_vertices.shape))  #(3, 7, 8, 3)
...
link_vertices =  np.zeros((trajectory.shape[0], num_links, 8, 3), dtype=float) 

# Create data for classification: 
dists =  np.zeros((trajectory.shape[0], num_links, 8, 3), dtype=float) 
for conf_no in range(len(trajectory)):  #50 for mpinets
    ...
    #     link_pos = env.client_id.getLinkState(panda, joint_no, computeForwardKinematics=1)[0]
    #     print(link_pos)
    #     grad = None
